...
import {K8s_Secret_Generic_StationHandler} from '../../../../lib/stations/k8s';
import {AssertHelper} from '../../../helpers/AssertHelper';
import {IOC} from '../../../../lib/services';
import {ChildProcessUtil} from '../../../../lib/utils';
import {resolve} from 'path';

@suite class K8s_Secret_Generic_StationHandler_TestSuite {
    async before() {
    ...
    @test async validationPassed(): Promise<void> {
        const map = <IRailwayMap> {
            ...
            station: {
                ...
                options: {
                    ...
                    files: [
                        resolve('./test/unit/stations/k8s/K8s_Secret_Generic_StationHandler_TestSuite.ts'),
                    ],
                }
            }
        };
        ...
        const railway = new A6sRailway(map)
            .register(new K8s_Secret_Generic_StationHandler());

        await railway.execute();
    }

    @test async validationFailed(): Promise<void> {
        const k8s_Secret_StationHandler = new K8s_Secret_Generic_StationHandler();
